...
- V2.5 只把未来平台扩展先登记成只读面板，不实现 cron、voice、profiles、remote login、gateway / messaging 或 auto update 的执行能力。
...
V2.5 起，Sage Agent 在 Settings 中只读展示未来平台扩展登记面板，用来说明哪些 Hermes 风格 surface 仍处于提案或 planned 阶段。
...
- `planned`：只登记，不执行。
- `blocked`：当前主线不做，原因必须可见。
- `proposed`：需要先有单独提案。

约束：

- 这些登记对象只用于解释未来扩展边界，不代表已经实现对应能力。
- Settings 只读展示这些信息，不提供执行、调度、连接、授权或持久化入口。
- 任何真正的平台执行能力都必须先通过单独提案和任务拆分。
