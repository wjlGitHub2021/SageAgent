...
## DEC-0017：V2.2 先落本地记忆注册表

状态：accepted

决策：

V2.2 的跨会话记忆底座先实现为本地记忆 registry + CRUD API + audit trail，并由 supervisor 运行时读取可用记忆注入上下文。

约束：

- 记忆条目和审计记录使用 `@sage/shared` 的共享 domain types。
- 记忆管理入口由 Web 工作台承载，当前以本地单用户为边界。
- 运行时只消费记忆上下文，不绕过 audit trail 直接变更持久化状态。
- 未来如果引入文件 / 数据库持久化，可以替换 registry 后端，但不改变上层 memory contract。
...
## DEC-0019：V2.4 先落 provider registry 与 entry surface 合同

状态：accepted

决策：

V2.4 不直接接入第二个真实 provider，也不实现 desktop app。当前先把现有 DeepSeek-only 状态包装成 provider registry 快照，并新增 Web active / Desktop planned 的 entry surface 快照，让 UI、API 和 shared types 先摆脱“单 DeepSeek 状态槽”的结构限制。

约束：

- DeepSeek 仍是唯一真实 provider。
- `providerId` 在新建 run 时显式记录为 `deepseek`；旧 run settings 没有该字段时按 DeepSeek 兼容读取。
- 自动 fallback 在 registry 中显示为 `disabled`，不能静默切换或假装存在第二 provider。
- Desktop 只登记为 planned surface，不创建桌面封装、不改发布流。
- Provider registry、fallback 和 entry surface 文案必须进入双语 Settings UI。
